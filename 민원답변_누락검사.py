...
NAME_PATTERNS = [
    ...
    r'[,、]\s*([가-힣]{2,4})\s*\)',                       # (전화번호, 담당자명)
    r'담당자\s*[:：]\s*([가-힣]{2,4})',                   # 담당자:담당자명
    r'담당\s*[:：]\s*([가-힣]{2,4})',                     # 담당:담당자명
    r'담당자\s+([가-힣]{2,4})\s*\(',                      # 담당자 담당자명(
    r'담당\s+([가-힣]{2,4})\s*\(',                        # 담당 담당자명(
...
# 전화번호 추출 패턴
TEL_PATTERNS = [
    r'[☎☏]\s*(0\d{1,2}[-\s]?\d{3,4}[-\s]?\d{4}(?:[,\s]*\d{4})*)',  # ☎0XX-XXXX-XXXX,XXXX
    r'(0\d{1,2}[-\s]?\d{3,4}[-\s]?\d{4})',                          # 0XX-XXXX-XXXX
    r'\((\d{3}[-\s]?\d{4})\)',                                      # (XXX-XXXX)
    r'[☎☏]\s*(\d{3}[-\s]?\d{4})',                                  # ☎XXX-XXXX
]
